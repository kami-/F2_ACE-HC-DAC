...
_b = 0;_waitTime = 0;_r = 0;_t = 0.1;_d = 0;_a = "";_h = "";_oldTime = 0;_conditions = [];_aid = _this select 2;_sc = 0;
...
_articolor = DAC_Marker_Val select 14;_side = ["EAST","WEST","GUER","CIV"];_cv = 0;_cw = 1;_z = 0;_aMC = [];_fs = "";
...
if(!(isServer || (name player == f_HC_name))) then
{
	...
	else
	{
		...
		else
		{
			...
			_m  = format["_awp%1%2",_group,time];_ma = createmarker [_m, _val2];_ma setMarkerShape "ELLIPSE";
			_ma setMarkerSize [(_val4 / 2),(_val4 / 2)];_ma setMarkerColor "ColorBlack";_ma setMarkerBrush "DiagGrid";
			sleep 5;
			deletemarker _ma;
		};
	};
...
else
{
	...
	{			
		...
		if(_c > 0) then
		{
			...
			if(_fs == -1) then {_color = (_articolor select 4)} else {_color = (_articolor select _fs)};
			...
			if(count _condi > 0) then
			{
				if(call compile (_condi select 0)) then
				{
					if(count _condi > 1) then
					{
						...
						if(format["%1",DAC_Support_Logic] != scalar) then {DAC_Support_Logic setVariable ["arti_error", (_condi select 1), true]};
						sleep 1;
						if(format["%1",DAC_Support_Logic] != scalar) then {DAC_Support_Logic setVariable ["arti_error", -1, true]};
					};
				}
				else
				{
					if(format["%1",DAC_Support_Logic] != scalar) then {DAC_Support_Logic setVariable ["arti_error", 11, true]};
			...
			while {_count > 0} do
			{
				...
				if(count _conditions == 0) then
				{
					...
					{
						_count = 0;
						if(format["%1",DAC_Support_Logic] != scalar) then {DAC_Support_Logic setVariable ["arti_error", 14, true]};
						sleep 1;
						if(format["%1",DAC_Support_Logic] != scalar) then {DAC_Support_Logic setVariable ["arti_error", -1, true]};
					}
					else
				...
				else
				{
					...
					if(DAC_Marker > 0) then
					{
						if((DAC_Marker_Val select 12) > 0) then
						{
							...
							_m  = format["_bwp%1%2",_group,time];_mb = createmarker [_m, [0,0]];_mb setMarkerShape "RECTANGLE";
							_mb setMarkerSize [0,0];_mb setMarkerColor _color;
							_m  = format["_awp%1%2",_group,time];_ma = createmarker [_m, _basePos];_ma setMarkerShape "ELLIPSE";
							_ma setMarkerSize [0,0];_ma setMarkerColor "ColorBlack";_ma setMarkerBrush "DiagGrid";
							if(count _this < 5) then
							{
								_ma setMarkerSize [_d,_d];_r = random 360;
								_p = [(_basePos Select 0) + (Sin (_r) * _d),(_basePos Select 1) + (Cos (_r) * _d),0];
								_mb setMarkerSize [(_d / 3),(_d / 8)];
							}
							else
							{
								_d = _val9;
								_ma setMarkerSize [_d,_d];_r = random 360;_l = random _d;
					...
					while{_ca < count (_conditions select 1)} do
					{
						...
						if(DAC_Marker > 0) then
						{
							if((DAC_Marker_Val select 12) > 0) then
							{
								_m = format["_h%1%2",_artiUnit,time];_mc = createmarkerlocal [_m, [0,0]];_mc setMarkerTypelocal "Destroy";
		...
		if(_count == 0) then
		{
			if(DAC_Marker > 0) then
			{
				if((DAC_Marker_Val select 12) > 0) then
				{
					_m  = format["_awp%1%2",_group,time];_ma = createmarker [_m, position _target];_ma setMarkerShape "ELLIPSE";
					_ma setMarkerSize [_d,_d];_ma setMarkerColor "ColorRed";_ma setMarkerBrush "DiagGrid";
					_t = ((getMarkerSize _ma) select 0);
					while{_t > 0} do
					{
						_t = _t - 2;
						_ma setMarkerSize [_t,_t];
						sleep 0.03;
					};
					deletemarker _ma;
				};
