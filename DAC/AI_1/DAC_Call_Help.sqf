...
private [
			...
			"_leader","_countradio","_tempgroups","_groupside","_uArray","_DAC_Temp_Groups",
			...
			_run = true;_leader = objNull;_countradio = DAC_Radio_Max;_TempAll = DAC_All_Groups;_DAC_Temp_Groups = [];
					...
					if(({alive _x} count units _grouponfire) > 0) then
					{
						...
						else
						{
							...
							else
							{
								...
								|| (_hgroup in DAC_No_Support) || ((_hleader distance _troublepos) > _rw)
