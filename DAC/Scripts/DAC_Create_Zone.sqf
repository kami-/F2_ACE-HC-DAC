...
if(!((isServer) || (name player == f_HC_name))) exitwith {};
...
if((DAC_Com_Values select 1) == 1) then
{
	...
	_text2 = "<t color='#CCCCCC' size='0.8' shadow='1' shadowColor='#666666' align='left'> is initialized ...</t><br/>";
		...
		else
		{
			...
			else
			{
				...
				else
				{
					...
					sleep (random 0.3);
					waituntil {((format["%1",isnull DAC_StartDummy] != "BOOL") && ((getpos DAC_StartDummy select 0) > 1))};
				};
			};
		};
