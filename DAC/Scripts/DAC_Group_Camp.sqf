...
private [
			...
			"_gc","_allobj","_campSet","_setSkill","_setFleeing","_setrank","_soldier","_leaderman","_eman","_run",
			...
			_vehcargo = 0;_sj = 0;_m = "";_mp = [];_TempList = [];_EventList = [];_TempUnitsA = [];_randomcount = random 9999;
			...
			_we = 0;_ab = "";_ammoarray = [];_wd = 0;_mf = "";_gc = 0;_allobj = [];_campSet = [];_setSkill = 0;_setFleeing = 0;
	...
	while {_run > 0} do
	{
		switch (_run) do
		{
						...
						if((count _TempUnitsB > 0) && (!(((_TempUnitsB select 0) select 0) == "NULL"))) then
						{
							...
							else
							{
								...
								else
								{
									...
									_fire inflame true;_startpos = [(getpos _flag select 0),(getpos _flag select 1)+20,0];_camp setdir (((_TempUnitsB select 0) select 2) select 3);
									...
									if(DAC_Marker > 0) then
									{
										...
										{
											_m = format["m%1",_flag];_mf = createmarkerlocal [_m, position _flag];_mf setMarkerTypelocal "Flag1";
									...
									//_startpos = [3,_wpstart,[50,50],0] call DAC_fFindPos;
			...
			case 6:	{
						...
						else
						{
							...
							if(_sj == 0) then
							{
								...
								_leaderman setTargetAge "10 MIN";
			...
			case 8:	{
						...
						_group addWaypoint [position _leaderman, 0];_group addWaypoint [position _leaderman, 0];_group addWaypoint [[10,0], 0];_group addWaypoint [[100,0], 0];
						if((typeName _GroupVar) == "ARRAY") then
						{
							if(count _groupvar > 0) then
							{
								...
								{
									_vA set [_sj,((getpos (_groupvar select _sj)) select 0)];_sj = _sj + 1;
								};
							};
						};
			...
			case 9	:{
						...
						if(DAC_Marker_Val select 3 > 0) then {[_leaderman,1,_group] spawn DAC_fGroupMarker};
						...
						[[_group,_DAC_Z_Set,_groupende],_wparray,5,_camparray,_HeloArray,_groupside]execVM format["%1DAC_Move_Camp.sqf",DAC_AI_Path];
