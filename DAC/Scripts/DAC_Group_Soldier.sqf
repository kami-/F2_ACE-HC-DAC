...
private [
			...
			_sy = 0;_unittyp = "";_wpstart = [];_setSkill = 0;_leaderman = objNull;_setFleeing = 0;_setrank = "";_soldier = "";
		...
		switch (_run) do
		{
			...
			case 3:	{
						...
						if(!(DAC_InSpawn)) then
						{
							if(_vehicleval) then
							{
								_startpos = [(getpos _groupvehicle select 0) + 10,(getpos _groupvehicle select 0),0];
							}
							else
							{
			...
			case 7:	{
						...
						if(_sj == 0) then
						{
							...
							_leaderman setunitpos "up";_sj = 1;_run = 4;
							sleep 1;
						}
						else
						{
							_eman setspeedmode "full";_eman setbehaviour "aware";_eman setunitpos "up";_run = 4;
							sleep 1;
						};
					};
			case 8:	{
						...
						if(_vehicleval) then
						{
							{_x moveincargo _groupvehicle;[_x] ordergetin true} foreach units _group;
							{if(!(_x in _groupvehicle)) then {deletevehicle _x}} foreach units _group;
							_res = 6;[[_group,(_groupcount select 4),_groupende],_harray,6,_maxrad,_vehiclepara,_startid] spawn DAC_fMoveToVH;
						}
						else
						{
			...
			case 9:	{
						...
						if(DAC_Marker_Val select 3 > 0) then {[_leaderman,1,_group] execVM "DAC\Marker\DAC_GroupMarker.sqf"};
