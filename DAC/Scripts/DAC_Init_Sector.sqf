...
if(local player) then
{
	...
	{
		DAC_Sectors set [_i, 0];_z = ((_zones select _i) select 0);
		[_z,_i] spawn DAC_fSectorCheck;_i = _i + 1;
	};

	{DAC_Sector_Values set [count DAC_Sector_Values,_x]} foreach [_eS,_fS];
	if(DAC_Sector_Values select 6) then {execVM "DAC\Scripts\DAC_Hint_Sectors.sqf"};
};
sleep 1;
if(isServer || (name player == HCName)) then
{
	_i = 0;
	while {_i < count _zones} do
	{
		_z = ((_zones select _i) select 0);_s = ((_zones select _i) select 1);
		if(_s != "") then {[_z,_s] execVM "DAC\Scripts\DAC_Sector_Action.sqf"};
		_i = _i + 1;
	};
};