...
if((isServer || (name player == f_HC_name)) && (({((isPlayer _x) && (local _x))} count DAC_Players) == 0)) then
{
...
else
{
	...
	else
	{
		...
		/*
		if((TriggerText _zone) != "") then
		{
			_r = random 100;_m = format["_tx%1%2%3",_zid,_i,_r];
			_za = createmarkerlocal [_m, [(position _zone select 0)-5,(position _zone select 1)]];_za setMarkerShapelocal "ICON";
			_za setMarkerTypelocal "DOT";_za setMarkerSizelocal [0,0];_za setMarkerColorlocal "ColorBlack";_za setMarkerTextlocal (TriggerText _zone);
		};
		*/
