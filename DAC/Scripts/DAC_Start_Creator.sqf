...
if(DAC_Fast_Init) then {if((isServer || (name player == f_HC_name)) && (local player)) then {_vD = ViewDistance;_fg = fog;0 setFog 1;setViewDistance 5}};
...
if(DAC_Fast_Init) then {if((isServer || (name player == f_HC_name)) && (local player)) then {setViewDistance _vD;0.5 setFog _fg}};